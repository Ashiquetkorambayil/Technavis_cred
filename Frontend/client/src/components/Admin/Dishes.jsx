...
const handleDelete = async(id)=>{
  ...
  return (
    ...
    <div key={index} className="col-12 my-1 p-2 montserrat-400" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#c3c3d1',borderRadius:'32px' }}>
        <div className="items">
        <img className='avatar' src={`http://localhost:8000/images/${items.image}`} alt='pic' />
        </div>
       ...
       <h5 className='text-black item-text'>₹ {items.price}</h5>
        ...
        <div className='items'>
        <h5 className='text-black item-text'>{getCategories.find((item )=> item._id === items.categories)?.categories}</h5>
        </div>

        <div className='items' style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Tooltip title='Edit'>
          <Link>
        ...
        <input
              ...
              className='mb-3'
            />
            <input placeholder='Dish' type="text" style={{width:'100%'}} onChange={(e)=> setDishes(e.target.value)} />
           <select onChange={(e)=> setCategories(e.target.value)} value={categotries} className='my-3' style={{width:'100%'}} name="" id="">
            ...
            <input placeholder='Price' type="number" style={{width:'100%'}} onChange={(e)=> setPrice(e.target.value)} />
        </Modal.Body>
        <Modal.Footer>
        ...
        <Modal.Body>
            <input  style={{ width: "100%", margin: "10px" }}
            type="file"
            onChange={(e) => setFile(e.target.files[0])} />
            <input value={getDishesById.dishes} name='dishes' type="text" style={{width:'100%'}} onChange={(e)=>handleUpdateChange(e)} />

            <select onChange={(e)=> setGetCategoriesById(e.target.value)} value={getCategoriesById} className='my-3' style={{width:'100%'}} name="" id="">
            <option value="">Select..</option>
            {getCategories.map((items, index)=>(
                    <option key={index} value={items._id}>{items.categories}</option>
            ))}
            ...
            <input value={getDishesById.price} name='price' type="number" style={{width:'100%'}} onChange={(e)=>handleUpdateChange(e)} />
        </Modal.Body>
        <Modal.Footer>
